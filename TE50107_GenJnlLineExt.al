    ...
    fields
    {
        ...
        field(50103; "IC Bal. Account No."; Code[20]) //G014
        {
            ...
            var
                ...
                BankAcc2: Record "Bank Account";
            begin
                if "IC Bal. Account No." <> '' then begin
                    ICPathDetail.Reset();
                    ICPathDetail.SetRange("Path Code", "IC Path Code");
                    if ICPathDetail.FindLast() then begin
                        Company2 := ICPathDetail."To Company";

                        IF "IC Bal. Account Type" = "IC Bal. Account Type"::"Bank Account" then begin
                            BankAcc2.ChangeCompany(Company2);
                            if not BankAcc2.get("IC Bal. Account No.") then
                                Error('Bank Account %1 does not exists in %2', "IC Bal. Account No.", Company2);
                        end;

                        IF "IC Bal. Account Type" = "IC Bal. Account Type"::"G/L Account" then begin
                            GLAcc2.ChangeCompany(Company2);
                            if not GLAcc2.get("IC Bal. Account No.") then
                                Error('G/L Account %1 does not exists in %2', "IC Bal. Account No.", Company2);
                        end;
                    end;
                end;
            end;
