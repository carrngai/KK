report 50103 "General Journal - Test Ext"
{
    DefaultLayout = RDLC;
    RDLCLayout = './R50103_TestReportExt.rdl';
    ...
    dataset
    {
        dataitem("Gen. Journal Batch"; "Gen. Journal Batch")
        {
            ...
            column(ShowPostingGroup; ShowPostingGroup) //G002
            {
            }
            dataitem("Integer"; "Integer")
            {
                ...
                dataitem("Gen. Journal Line"; "Gen. Journal Line")
                {
                    ...
                    column(DrAmt_GenJnlLine; "Debit Amount") //G002
                    {
                    }
                    column(CrAmt_GenJnlLine; "Credit Amount") //G002
                    {
                    }
                    column(CurrFactor_GenJnlLine; "Gen. Journal Line"."Currency Factor") //G002
                    {
                    }
                    ...
                    dataitem(AppliedCLE; "Cust. Ledger Entry") //G002
                    {
                        ...
                        column(AppliestoID_AppliedCLE; "Applies-to ID")
                        {
                        }
                        ...
                        column(Amount_AppliedCLE; Amount)
                        {
                        }
                        column(AmountLCY_AppliedCLE; "Amount (LCY)")
                        {
                        }
                        column(AmounttoApply_AppliedCLE; "Amount to Apply")
                        {
                        }
                        column(CLEexist; CLEexist)
                        {
                        }
                    ...
                    dataitem(AppliedVLE; "Vendor Ledger Entry")
                    {
                        ...
                        column(Amount_AppliedVLE; Amount)
                        {
                        }
                        column(AmountLCY_AppliedVLE; "Amount (LCY)")
                        {
                        }
                        column(AmounttoApply_AppliedVLE; "Amount to Apply")
                        {
                        }
                        column(VLEexist; VLEexist)
                        {
                        }
                        trigger OnPreDataItem()
                        begin
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ShowPostingGroup; ShowPostingGroup) //G002
                    {
                        ApplicationArea = All;
                        Caption = 'Show Posting Group';
                        ToolTip = 'Specifies if you want posting group information for the journal lines to be included in the report.';
                    }
                }
            }
        }
    ...
    var
        ...
        GeneralJnlTestCap: Label 'General Journal - Ready to Post';
            ...
            if (CurrentICPartner <> '') and ("Gen. Journal Line"."IC Direction" = "Gen. Journal Line"."IC Direction"::Outgoing) then begin
                            ...
                            if ICGLAccount.Get("Gen. Journal Line"."IC Partner G/L Acc. No.") then
                                if ICGLAccount.Blocked then
                                    AddError(
                                      StrSubstNo(
                                        Text032,
                                        ICGLAccount.FieldCaption(Blocked), false, "Gen. Journal Line".FieldCaption("IC Partner G/L Acc. No."),
                ...
                if "Gen. Journal Line"."IC Partner G/L Acc. No." <> '' then begin
                    if "Gen. Journal Line"."IC Direction" = "Gen. Journal Line"."IC Direction"::Incoming then
                        AddError(
                          StrSubstNo(
                            Text069, "Gen. Journal Line".FieldCaption("IC Partner G/L Acc. No."), "Gen. Journal Line".FieldCaption("IC Direction"), Format("Gen. Journal Line"."IC Direction")));
                    if CurrentICPartner = '' then
                        AddError(
                          StrSubstNo(
                            Text070, "Gen. Journal Line".FieldCaption("IC Partner G/L Acc. No.")));
                end;
        end;
    end;
